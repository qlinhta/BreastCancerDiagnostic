...
fig, ax = plt.subplots(2, 2, figsize=(20, 15))
sns.kdeplot(X_test[y_pred == 0]['smoothness_mean_log'], label='Benign', shade=True, linestyle='--', ax=ax[0, 0])
sns.kdeplot(X_test[y_pred == 1]['smoothness_mean_log'], label='Malignant', shade=True, linestyle='--', ax=ax[0, 0])
ax[0, 0].set_title('Smoothness Mean')
ax[0, 0].set_xlabel('Log Scale of Smoothness Mean')
ax[0, 0].set_ylabel('Predicted density')
ax[0, 0].legend()
sns.kdeplot(X_test[y_pred == 0]['texture_mean_log'], label='Benign', shade=True, linestyle='--', ax=ax[0, 1])
sns.kdeplot(X_test[y_pred == 1]['texture_mean_log'], label='Malignant', shade=True, linestyle='--', ax=ax[0, 1])
ax[0, 1].set_title('Texture Mean')
ax[0, 1].set_xlabel('Log Scale of Texture Mean')
ax[0, 0].set_ylabel('Predicted density')
ax[0, 1].legend()
sns.kdeplot(X_test[y_pred == 0]['area_mean'], label='Benign', shade=True, linestyle='--', ax=ax[1, 0])
sns.kdeplot(X_test[y_pred == 1]['area_mean'], label='Malignant', shade=True, linestyle='--', ax=ax[1, 0])
ax[1, 0].set_title('Area Mean')
ax[1, 0].set_xlabel('Scale of Area Mean')
ax[0, 0].set_ylabel('Predicted density')
ax[1, 0].legend()
sns.kdeplot(X_test[y_pred == 0]['concavity_mean'], label='Benign', shade=True, linestyle='--', ax=ax[1, 1])
sns.kdeplot(X_test[y_pred == 1]['concavity_mean'], label='Malignant', shade=True, linestyle='--', ax=ax[1, 1])
ax[1, 1].set_title('Concavity Mean')
ax[1, 1].set_xlabel('Scale of Concavity Mean')
ax[0, 0].set_ylabel('Predicted density')
ax[1, 1].legend()
# plt.savefig('output_plots/LR_density_chart_with_prediction.png')
plt.show()

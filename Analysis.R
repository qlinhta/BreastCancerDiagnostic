...
ggpairs(df[,c(2:11,1)], aes(color=diagnosis, alpha=0.75), lower=list(continuous="smooth"))+ theme_bw()+
  labs(title="Mean")+theme(plot.title=element_text(face='bold',color='black',hjust=0.5,size=10))
ggpairs(df[,c(12:21,1)], aes(color=diagnosis, alpha=0.75), lower=list(continuous="smooth"))+ theme_bw()+
  labs(title="SE")+theme(plot.title=element_text(face='bold',color='black',hjust=0.5,size=10))
ggpairs(df[,c(22:31,1)], aes(color=diagnosis, alpha=0.75), lower=list(continuous="smooth"))+ theme_bw()+
  labs(title="Worst")+theme(plot.title=element_text(face='bold',color='black',hjust=0.5,size=10))
...
b1 <- boxplot2g(df$radius_worst, df$perimeter_mean, df$diagnosis, smooth = loess, NULL, NULL) +
  labs(title="Highly correlated features", subtitle = "Perimeter mean vs. Radius worst", x="Radius worst", y="Perimeter mean") + theme_bw()
b2 <- boxplot2g(df$area_worst, df$radius_worst, df$diagnosis, smooth = loess, NULL, NULL) +
  labs(title="Highly correlated features", subtitle = "Area worst vs. Radius worst", x="Radius worst", y="Area worst") + theme_bw()
b3 <- boxplot2g(df$texture_mean, df$texture_worst, df$diagnosis, smooth = loess, NULL, NULL) +
  labs(title="Highly correlated features", subtitle = "Texture mean vs. Texture worst", x="Texture worst", y="Texture mean") + theme_bw()
b4 <- boxplot2g(df$area_worst, df$perimeter_mean, df$diagnosis, smooth = loess, NULL, NULL) +
  labs(title="Highly correlated features", subtitle = "Perimeter mean vs. Area worst", x="Area worst", y="Perimeter mean") + theme_bw()
grid.arrange(b1, b2, b3, b4, ncol=2)

b9 <- boxplot2g(df$fractal_dimension_worst, df$area_se, df$diagnosis, smooth = loess, NULL, NULL) +
  labs(title="Low correlated features", subtitle = "Area SE vs. Fractal dimmension worst", x="Fractal dimmension worst", y="Area SE") + theme_bw()
b10 <- boxplot2g(df$fractal_dimension_worst, df$radius_se, df$diagnosis, smooth = loess, NULL, NULL) +
  labs(title="Low correlated features", subtitle = "Radius SE vs. Fractal dimmension worst", x="Fractal dimmension worst", y="Radius SE") + theme_bw()
b11 <- boxplot2g(df$texture_mean, df$smoothness_mean, df$diagnosis, smooth = loess, NULL, NULL) +
  labs(title="Low correlated features", subtitle = "Smoothness mean vs. Texture mean", x="Texture mean", y="Smoothness mean") + theme_bw()
b12 <- boxplot2g(df$perimeter_worst, df$fractal_dimension_se, df$diagnosis, smooth = loess, NULL, NULL) +
  labs(title="Low correlated features", subtitle = "Fractal dimmension SE vs. Perimeter worst", x="Perimeter worst", y="Fractal dimension SE") + theme_bw()
...
b14 <- boxplot2g(df$radius_worst, df$perimeter_worst, df$diagnosis, smooth = loess, NULL, NULL) +
    labs(title="Highly correlated features", subtitle = "Perimeter worst vs. Radius worst", x="Radius worst", y="Perimeter worst") + theme_bw()
...
b16 <- boxplot2g(df$radius_worst, df$area_mean, df$diagnosis, smooth = loess, NULL, NULL) +
    labs(title="Highly correlated features", subtitle = "Area mean vs. Radius worst", x="Radius worst", y="Area mean") + theme_bw()
